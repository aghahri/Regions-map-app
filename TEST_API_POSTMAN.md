# راهنمای تست API با Postman

## API Endpoint: دریافت نام محله از مختصات

### URL:
```
http://171.22.27.42/api/neighborhood
```

---

## روش 1: GET (ساده‌تر) - پیشنهادی

### تنظیمات در Postman:

1. **Method**: `GET` را انتخاب کنید

2. **URL**: 
   ```
   http://171.22.27.42/api/neighborhood?lat=35.6892&lon=51.3890
   ```

3. **Params** (در تب Params):
   - `lat`: `35.6892` (عرض جغرافیایی)
   - `lon`: `51.3890` (طول جغرافیایی)
   - `map_id`: (اختیاری) شناسه نقشه خاص

4. روی **Send** کلیک کنید

---

## روش 2: POST (JSON Body)

### تنظیمات در Postman:

1. **Method**: `POST` را انتخاب کنید

2. **URL**: 
   ```
   http://171.22.27.42/api/neighborhood
   ```

3. **Headers**:
   ```
   Content-Type: application/json
   ```

4. **Body** → **raw** → **JSON** را انتخاب کنید:
   ```json
   {
     "lat": 35.6892,
     "lon": 51.3890
   }
   ```

5. روی **Send** کلیک کنید

---

## روش 3: POST (Form Data)

### تنظیمات در Postman:

1. **Method**: `POST`

2. **URL**: 
   ```
   http://171.22.27.42/api/neighborhood
   ```

3. **Body** → **x-www-form-urlencoded**:
   | Key | Value |
   |-----|-------|
   | `lat` | `35.6892` |
   | `lon` | `51.3890` |

4. روی **Send** کلیک کنید

---

## Response موفق (200 OK):

```json
{
  "success": true,
  "neighborhood": "داوودیه",
  "district": "3",
  "city": "تهران",
  "map_id": "20241128_120000_123456",
  "map_name": "نقشه تهران",
  "tootapp_url": "https://tootapp.ir/join/Tehran3Da",
  "coordinates": {
    "lat": 35.6892,
    "lon": 51.3890
  }
}
```

---

## Response خطا - محله پیدا نشد (404):

```json
{
  "success": false,
  "error": "محله‌ای برای این مختصات پیدا نشد",
  "coordinates": {
    "lat": 35.6892,
    "lon": 51.3890
  }
}
```

---

## مثال‌های مختصات تهران:

### میدان آزادی:
```
lat: 35.6997
lon: 51.3381
```

### میدان ولیعصر:
```
lat: 35.7614
lon: 51.4113
```

### داوودیه:
```
lat: 35.6892
lon: 51.3890
```

### میدان انقلاب:
```
lat: 35.6961
lon: 51.4231
```

---

## تصویر راهنما در Postman (GET):

```
┌─────────────────────────────────────────────────────┐
│ [GET ▼] http://171.22.27.42/api/neighborhood        │ [Send]
├─────────────────────────────────────────────────────┤
│ Params ● Query Params                               │
│                                                      │
│ Key          │ Value        │ Description           │
│ lat          │ 35.6892      │                      │
│ lon          │ 51.3890      │                      │
│ map_id       │ (optional)    │                      │
└─────────────────────────────────────────────────────┘
```

---

## تصویر راهنما در Postman (POST JSON):

```
┌─────────────────────────────────────────────────────┐
│ [POST ▼] http://171.22.27.42/api/neighborhood       │ [Send]
├─────────────────────────────────────────────────────┤
│ Headers                                              │
│ Content-Type: application/json                      │
├─────────────────────────────────────────────────────┤
│ Body ● raw ▼ JSON                                    │
│                                                      │
│ {                                                    │
│   "lat": 35.6892,                                    │
│   "lon": 51.3890                                     │
│ }                                                    │
└─────────────────────────────────────────────────────┘
```

---

## تست سریع با curl:

```bash
# GET
curl "http://171.22.27.42/api/neighborhood?lat=35.6892&lon=51.3890"

# POST JSON
curl -X POST http://171.22.27.42/api/neighborhood \
  -H "Content-Type: application/json" \
  -d '{"lat": 35.6892, "lon": 51.3890}'
```

---

## نکات مهم:

1. ✅ **lat** = عرض جغرافیایی (latitude)
2. ✅ **lon** = طول جغرافیایی (longitude)
3. ✅ اگر `map_id` ندهید، در همه نقشه‌ها جستجو می‌کند
4. ✅ اگر محله پیدا نشد، `404` با پیام خطا برمی‌گرداند

---

## اگر خطا می‌گیرید:

### 404 Not Found:
- مطمئن شوید که API روی سرور deploy شده
- بررسی کنید که `/api/neighborhood` endpoint موجود است

### 500 Internal Server Error:
- Logs را بررسی کنید: `sudo journalctl -u regions-map-app -n 50`

### Connection Error:
- مطمئن شوید سرور در دسترس است
- بررسی کنید که Nginx و Gunicorn در حال اجرا هستند

---

## مثال کامل تست:

1. Postman را باز کنید
2. Method: `GET`
3. URL: `http://171.22.27.42/api/neighborhood?lat=35.6892&lon=51.3890`
4. Send را بزنید
5. Response را ببینید

ساده! 🎉


